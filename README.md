...
- prompt
- language
- is_correct (considers only exact string matches)
...
- Long experiments may take several hours depending on the GPU and models used.
- Intermediate results are saved regularly during runtime.
- Due to the nature of LLMs, the results are not entirely deterministic.
- The target class annotation in the dataset is not always unambiguous, which means that some noise in the dataset cannot be completely ruled out.
